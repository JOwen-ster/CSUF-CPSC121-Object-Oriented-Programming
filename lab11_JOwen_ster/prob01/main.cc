...
int main() {
  ...
  // =======================================================
for (std::vector<double>::iterator it = weights.begin();
        it != weights.end();
        it++)
  std::cout << *it << " ";
  ...
  //    map.
  // ======================================================
  for(std::map<std::string, double>::iterator it = populations.begin(); it != populations.end(); it++){
    for(std::map<std::string, std::string>::iterator it2 = abbrevs.begin(); it2 != abbrevs.end(); it2++){
      if(it2->first == it->first) std::cout << "Population of " << it2->second << ": " << it->second << " million\n";
    }
  }
  ...
  // ======================================================
fleet.insert(fleet.end(), {p3, p4, p5});
  ...
  // ======================================================
// std::cout << "121 passengers flying to Fullerton\n220 passengers flying to Atlanta\n75 passengers flying to Guatemala City";
  
for(std::vector<Plane>::iterator it = fleet.begin(); it != fleet.end(); it++){
  if(it->GetSeatCount() == it->GetPassengerCount()){
    std::cout << it->GetSeatCount() << " passengers flying to " << it->GetDestination() << "\n";
  }
}
